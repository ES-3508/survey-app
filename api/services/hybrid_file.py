...
def calculate_UN_for_point(E_point, N_point, UN_point,graivity, order, df_model):
    ...
    UN_new = point_array.dot(Z)+UN_point-graivity

    return UN_new

# Function to process a DataFrame of points and calculate UN for each
def calculate_UN_hybrid_for_dataframe(df_input, model_csv_path, order):
    ...
    for _, row in df_input.iterrows():
        ...
        UN_new = calculate_UN_for_point(E_point, N_point, UN_point,gravity, order, df_model)
        results.append([E_point, N_point, UN_point,gravity, UN_new])

    # Create a DataFrame with the results
    df_results = pd.DataFrame(results, columns=['East', 'North', 'Ellipse','Gravity', 'UN_new'])
...
if __name__ == '__main__':
    ...
    df_results = calculate_UN_hybrid_for_dataframe(df_input, model_csv_path, order)
    print(df_results)
